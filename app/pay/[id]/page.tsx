...
export default function PayPage() {
  ...
  async function handlePay() {
    ...
    try {
      ...
      if (!response.ok) {
        setMessageType("error")
        setMessage(data.error || "M-Pesa payment failed. Please try again.")
        setLoading(false)
        return
      }
        ...
        <div className="grid grid-cols-3 gap-6 max-w-4xl">
          ...
          <div className="col-span-2 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-800 mb-4">Ticket quantity</h2>
              <div className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border border-gray-100">
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setQuantity(q => Math.max(1, q - 1))}
                    className="w-10 h-10 bg-white rounded-lg border border-gray-200 text-gray-600 font-bold text-xl flex items-center justify-center hover:bg-gray-50 shadow-sm"
                  >−</button>
                ...
                <div className="flex flex-col items-end">
                  <span className="bg-green-100 text-green-700 text-xs font-bold px-3 py-1 rounded-full mb-1">
                    Available
                  </span>
                  <span className="text-xs text-gray-400">Max 10 tickets per student</span>
                </div>
              </div>
            </div>
